...
contract('Staking', ([
    ...
    it('#1 Deposit 5 tokens from alice -> After 0.5 months withdraw 3 tokens -> After 0.5 claim and deposit 3 tokens -> Increase time to the end of year, withdraw all tokens and claim', async () => {
        ...
        */
        ...
        /*expect(await tokenReward.balanceOf(alice)).to.be.bignumber.that.equals(rewards_for_alice)
        expect(await tokenReward.balanceOf(Staking.address)).to.be.bignumber.that.equals(REWARDS.sub(rewards_for_alice))
       ...
       // expect((await Staking.getPoolInfo())._globalCoefficient).to.be.bignumber.equals(((new BN(timestamp_withdraw_1)).sub(new BN(timestamp_deposit_1))).mul(MULTIPLIER).div(FIVE))
        */
        //!!!!!!!!
        ...
        /*glKoffM = ((new BN(timestamp_withdraw_1)).sub(new BN(timestamp_deposit_1))).mul(MULTIPLIER).div(FIVE)
        glKoffM = glKoffM.add(((new BN(timestamp_deposit_2)).sub(new BN(timestamp_withdraw_1))).mul(MULTIPLIER).div(TWO))
        ...
        let koeff = (((new BN(timestamp_withdraw_1)).sub(new BN(timestamp_deposit_1))).mul(MULTIPLIER).div(FIVE)).add(((new BN(timestamp_deposit_2)).sub(new BN(timestamp_withdraw_1))).mul(MULTIPLIER).div(TWO))
        //expect((await Staking.getPoolInfo())._globalCoefficient).to.be.bignumber.equals(koeff)  */
    ...
    it('#2 Deposit 5 tokens from Alice -> Increase time - 30days -> Deposit 10 tokens from Bob -> Increase time - 30days -> Claim', async () => {
       ...
       /* expect(await tokenStaked.balanceOf(alice)).to.be.bignumber.that.equals(STO.sub(FIVE))
        ...
        let rewards_alice = (((new BN(timestamp_deposit_bob)).sub(new BN(timestamp_deposit_alice))).mul(N).div(MULTIPLIER))
        rewards_alice = rewards_alice.add(((new BN(timestamp_claim_alice)).sub(new BN(timestamp_deposit_bob))).mul(N).mul(FIVE).div(TEN.add(FIVE)).div(MULTIPLIER))
      ...
      //  timestamp_claim_alice_1 = (await web3.eth.getBlock((await Staking.claim({ from: alice })).receipt.blockNumber)).timestamp 
