...
const MULTIPLIER  = new BN((10**20).toString());
...
const ZERO_TROI = new BN((0.3*10**18).toString());
const ZERO__TROI = new BN((0.03*10**18).toString());
const ZERO___TROI = new BN((0.003*10**18).toString());
...
contract('Staking', ([
    ...
    it('#0 check exeptions before initiating Staking and conditions after that', async () => {
        ...
        expect((await tokenReward.balanceOf(Staking.address))).to.be.bignumber.equals(REWARDS);
    ...
    it('#2 Deposit 5 tokens from Alice -> Increase time - 30days -> Deposit 10 tokens from Bob -> Increase time - 30days -> Claim', async () => {
        ...
        try {
            ...
            expect(Math.abs((await Staking.getUserInfo(bob)).globalCoefficientMinus.sub(koff))).to.be.lessThanOrEqual(1);
        }
        ...
        try {
            ...
            expect(Math.abs((await Staking.getPoolInfo())._globalCoefficient.sub(koff))).to.be.lessThanOrEqual(1);
        }
    ...
    it('#3 Deposit 10 tokens from Alice -> Increase time - 30days -> Deposit 10 tokens from Bob -> Increase time - 30days -> WITHDRAW 5 tokens for Alice -> Increase time - 30days -> WITHDRAW 5 tokens for bob -> Increase time - 30days -> CLAIM', async () => {
        ...
        expect((await Staking.getPoolInfo())._holdersAmount).to.be.bignumber.equals(_ZERO);
        expect((await tokenReward.balanceOf(Staking.address))).to.be.bignumber.equals(_ZERO);
    }) 
    ...
    it('#4 deposit for alice ang eva 10 tokens -> Increase time - 30days -> deposit for bob 5 tokens and withdraw for eva and alice 5 tokens ->  Increase time - 30days -> check claiming', async () => {
        ...
        await time.increase(time.duration.days(11*30));
    ...
    it('#8 test process whith the partial numbers', async () => {
        let rewards_alice, rewards_bob, timestamp_deposit_alice, timestamp_deposit_bob, timestamp_deposit_eva, timestamp_claim_alice, timestamp_claim_bob, timestamp_claim_eva, timestamp_withdraw_alice, timestamp_withdraw_bob, timestamp_withdraw_eva, timestamp_claim_alice_1, timestamp_claim_bob_1;

        await tokenReward.transfer(Staking.address, REWARDS, {from: owner});
        await Staking.initStaking(tokenStaked.address, tokenReward.address, devWallet, YEAR, { from: owner });

        timestamp_deposit_alice = (await web3.eth.getBlock((await Staking.deposit(ZERO___TROI, { from: alice })).receipt.blockNumber)).timestamp;
        let end = timestamp_deposit_alice + 60*60*24*30*13;

        expect((await Staking.getUserInfo(alice)).amount).to.be.bignumber.equals(ZERO___TROI);
        expect((await Staking.getUserInfo(alice)).start).to.be.bignumber.equals(timestamp_deposit_alice.toString());
        expect((await Staking.getUserInfo(alice)).globalCoefficientMinus).to.be.bignumber.equals(_ZERO);
        expect((await Staking.getUserInfo(alice)).assignedReward).to.be.bignumber.equals(_ZERO);

        expect((await Staking.getPoolInfo())._stakedSum).to.be.bignumber.equals(ZERO___TROI);
        expect((await Staking.getPoolInfo())._globalCoefficient).to.be.bignumber.equals(_ZERO);
        expect((await Staking.getPoolInfo())._lastUpdate).to.be.bignumber.equals(timestamp_deposit_alice.toString());
       
        expect(await tokenStaked.balanceOf(alice)).to.be.bignumber.that.equals(STO.sub(ZERO___TROI));
        expect(await tokenStaked.balanceOf(Staking.address)).to.be.bignumber.that.equals(ZERO___TROI);
        ...
        rewards_alice = (N.mul((new BN(timestamp_deposit_bob)).sub(new BN(timestamp_deposit_alice))).div(MULTIPLIER));
        let koff = ((new BN(timestamp_deposit_bob)).sub(new BN(timestamp_deposit_alice)).mul(MULTIPLIER).div(ZERO___TROI));
        ...
        expect((await Staking.getPoolInfo())._stakedSum).to.be.bignumber.equals(ZERO___TROI.add(TEN));
        expect((await Staking.getPoolInfo())._lastUpdate).to.be.bignumber.equals(timestamp_deposit_bob.toString());
        ...
        expect(await tokenStaked.balanceOf(alice)).to.be.bignumber.that.equals(STO.sub(ZERO___TROI));
        expect(await tokenStaked.balanceOf(bob)).to.be.bignumber.that.equals(STO.sub(TEN));
        expect(await tokenStaked.balanceOf(Staking.address)).to.be.bignumber.that.equals(ZERO___TROI.add(TEN));
        ...
        rewards_alice = rewards_alice.add(N.mul((new BN(timestamp_claim_alice)).sub(new BN(timestamp_deposit_bob))).mul(ZERO___TROI).div(ZERO___TROI.add(TEN)).div(MULTIPLIER));
        rewards_bob = (N.mul((new BN(timestamp_claim_bob)).sub(new BN(timestamp_claim_alice))).mul(TEN).div(ZERO___TROI.add(TEN)).div(MULTIPLIER)).add(N.mul((new BN(timestamp_claim_alice)).sub(new BN(timestamp_deposit_bob))).mul(TEN).div(ZERO___TROI.add(TEN)).div(MULTIPLIER));
        ...
        timestamp_withdraw_alice = (await web3.eth.getBlock((await Staking.withdraw(ZERO___TROI, { from: alice })).receipt.blockNumber)).timestamp; 
        
        koff = koff.add((new BN(timestamp_withdraw_alice)).sub(new BN(timestamp_deposit_bob)).mul(MULTIPLIER).div(ZERO___TROI.add(TEN)));
        ...
        timestamp_deposit_eva = (await web3.eth.getBlock((await Staking.deposit(ZERO__TROI, { from: eva })).receipt.blockNumber)).timestamp;
      
        expect((await Staking.getPoolInfo())._stakedSum).to.be.bignumber.equals(ZERO__TROI);
        expect((await Staking.getPoolInfo())._lastUpdate).to.be.bignumber.equals(timestamp_deposit_eva.toString());
        ...
        expect(await tokenStaked.balanceOf(eva)).to.be.bignumber.that.equals(STO.sub(ZERO__TROI));
        expect(await tokenStaked.balanceOf(Staking.address)).to.be.bignumber.that.equals(ZERO__TROI);
        ...
        timestamp_withdraw_eva = (await web3.eth.getBlock((await Staking.withdraw(ZERO__TROI, { from: eva })).receipt.blockNumber)).timestamp;
        timestamp_withdraw_alice = (await web3.eth.getBlock((await Staking.withdraw(ZERO___TROI, { from: alice })).receipt.blockNumber)).timestamp;
    
        rewards_eva = rewards_eva.add(((new BN(end)).sub(new BN(timestamp_deposit_alice))).mul(N).mul(ZERO__TROI).div(ZERO__TROI.add(ZERO___TROI)).div(MULTIPLIER));
        ...
        rewards_alice = rewards_alice.add(((new BN(end)).sub(new BN(timestamp_deposit_alice))).mul(N).mul(ZERO___TROI).div(ZERO__TROI.add(ZERO___TROI)).div(MULTIPLIER));
        try {
            expect(await tokenReward.balanceOf(eva)).to.be.bignumber.equals(rewards_eva);
        } catch {
            expect(Math.abs((await tokenReward.balanceOf(eva)).sub(rewards_eva))).to.be.lessThanOrEqual(2);
